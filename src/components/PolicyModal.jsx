...
const PolicyModal = (props) => {
    ...
    return(
        <>
            ...
            <div className='border border-[var(--light-border-color)] relative rounded-lg bg-gradient-to-b from-white to-[#97cfd0] flex flex-col gap-6 font-inter items-center justify-center h-auto w-11/12 md:w-3/4 lg:w-3/5 xl:w-7/12 2xl:w-1/2 px-10 py-8' id='new-order' onClick={(e) => e.stopPropagation()}>
                ...
                <div className="w-full overflow-y-auto h-64 md:h-72 lg:h-80 xl:h-[500px] max-h-screen flex flex-col items-start justify-start mb-14 text-left text-gray-600 text-xs md:text-sm lg:text-base xl:text-lg">
                    ...
                    {props.body?.map((item) => (<><p className='w-full'>{item}</p><br /></>))}
                        {/* <p className="w-full">
