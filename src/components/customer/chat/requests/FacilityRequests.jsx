    ...
    return(
        <>
            ...
            : requestableFacilities.length > 0 ? (requestableFacilities?.map((item,index) => (<div key={index} className='w-full hover:bg-gray-50 border-[var(--light-border-color)] h-20 flex items-center justify-start gap-2 cursor-pointer relative py-4' onClick={() => props.onClick(item)}>
                ...
                <div className='border rounded-full h-7 w-7 md:h-10 md:w-10 bg-gray-200 border-[var(--light-border-color)] ml-2 flex items-center justify-center'>
                    <PiBuildingApartmentFill className='text-gray-500 h-5 md:h-7 w-5 md:w-7' />
                </div>)}
                <p className='text-xs sm:text-sm md:text-base text-gray-400 truncate'>{item.name}</p>
