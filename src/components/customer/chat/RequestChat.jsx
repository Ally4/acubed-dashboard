...
const RequestChat = (props) => {
    ...
    const [selectedSection, setSelectedSection] = useState('Contacts') //Contacts or Request Page
    ...
    return(
        <div className='min-w-full min-h-96 h-full md:h-[450px] lg:h-[500px] grid grid-cols-[1fr] place-items-center md:grid-cols-[210px_1fr] lg:grid-cols-[250px_1fr] xl:grid-cols-[300px_1fr] lg:rounded-br-lg lg:rounded-bl-lg'>
            {/*Message Request */}
            
                <div className={`h-full w-full border-r border-[var(--light-border-color)] overflow-y-auto flex-col items-center justify-start ${selectedSection === 'Contacts' ? 'flex' : 'hidden'} md:flex`}>
                    ...
                    {showFacilities && <FacilityRequests onClick={(item)=>{
                        ...
                        setSelectedSection('Request Page')
                    }} />}
                ...
                <div className={`w-full h-full ${selectedSection === 'Request Page' ? 'flex' : 'hidden'} md:flex flex-col items-center justify-center gap-1 px-6 relative`}>
                        <IoMdArrowRoundBack className='absolute top-3 left-4 text-gray-700 h-8 w-8 cursor-pointer md:hidden' onClick={()=>{
                        ...
                        :
                         (<div className='flex flex-col items-center justify-center h-auto mb-3 w-full'>
                            <h3 className='text-gray-500 text-lg md:text-xl font-medium'>{selectedFacility?.name}</h3>
                            {requestSuccess != true && (<>
                                <input type='text' placeholder='Enter request message' value={requestMessage} className='focus:outline-none border border-[var(--light-border-color)] w-11/12 md:w-10/12 mb-4 px-2 py-2 rounded-md' onChange={handleMessageChange}/>
